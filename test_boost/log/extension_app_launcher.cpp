...
class app_launcher
    : public sinks::basic_formatted_sink_backend<
          ...
          sinks::synchronized_feeding /*< in order not to spawn too many
...
void init_logging() {
  ...
  sink->set_formatter(
      ...
      << boost::phoenix::bind(&severity_level_as_urgency,
                              logging::trivial::severity.or_none())
      << expr::if_(expr::has_attr(
             process_name))[expr::stream << " -a '" << process_name << "'"]
      << expr::if_(expr::has_attr(
             caption))[expr::stream
                       << " \""
                       << expr::char_decor(
                              shell_decorations)[expr::stream << caption]
                       << "\""]
      << " \""
...
void test_notifications() {
  ...
  BOOST_LOG_TRIVIAL(info) << logging::add_value(caption, "Caption text")
                          << "And this notification has caption as well";
}
//]
