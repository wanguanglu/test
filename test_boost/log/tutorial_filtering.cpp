...
void init() {
  ...
  logging::formatter fmt =
      expr::stream << std::setw(6) << std::setfill('0') << line_id
  ...
  sink->set_filter(severity >= warning || (expr::has_attr(tag_attr) &&
...
void init2() {
  ...
  sink->set_filter(
      phoenix::bind(&my_filter, severity.or_none(), tag_attr.or_none()));
