...
int main(int argc, char **argv) {
  ...
  printf("Run \"nbody_opengles -benchmark [-numbodies=<numBodies>]\" to "
         "measure performance.\n");
  showHelp();
      ...
      for (int i = 0; i < numDevsRequested; i++) {
        ...
        if (useHostMem) {
          if (!props.canMapHostMemory) {
            ...
            // cudaDeviceReset causes the driver to clean up all state. While
            // not mandatory in normal operation, it is good practice.  It is
            // also needed to ensure correct operation when the application is
            // being profiled. Calling cudaDeviceReset causes all profile data
            // to be flushed before the application exits
            cudaDeviceReset();
            exit(EXIT_SUCCESS);
          }
