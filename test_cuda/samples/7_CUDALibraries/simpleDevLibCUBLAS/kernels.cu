...
__global__ void invokeDeviceCublasSgemm(cublasStatus_t* returnValue, int n,
  ...
  status = cublasSgemm(cnpHandle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, d_alpha,
...
extern "C" void device_cublas_sgemm(int n, float alpha, const float* d_A,
  ...
  if (cudaMemcpy(d_params, &h_params, sizeof(SGEMMScalarParams),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
    fprintf(stderr, "!!!! host to device memory copy error\n");
    exit(EXIT_FAILURE);
  }

  // Launch cublasSgemm wrapper kernel.
  invokeDeviceCublasSgemm<<<1, 1>>>(d_status, n, &d_params->alpha, d_A, d_B,
                                    &d_params->beta, d_C);
  ...
  if (cudaMemcpy(&status, d_status, sizeof(cublasStatus_t),
