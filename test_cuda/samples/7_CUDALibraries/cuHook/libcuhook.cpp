...
#define CU_HOOK_PRINT(args...)                                                 \
  { fprintf(stderr, "CUDA Hook Library: " args); }
...
static void *real_dlsym(void *handle, const char *symbol) {
  static fnDlsym internal_dlsym =
      (fnDlsym)__libc_dlsym(dlopen("libdl.so.2", RTLD_LAZY), "dlsym");
...
struct HookInfo {
  ...
  HookInfo(const char *dl) {
    ...
    // Load the libcuda.so library with RTLD_GLOBAL so we can hook the function
    // calls
    handle = dlopen(dl, RTLD_LAZY | RTLD_GLOBAL);
    CU_HOOK_DIE_IF(!handle, "Failed to load libcuda.so, %s\n", dlerror());

    realFunctions[CU_HOOK_MEM_ALLOC] =
        real_dlsym(handle, CUDA_SYMBOL_STRING(cuMemAlloc));
    CU_HOOK_DIE_IF(!realFunctions[CU_HOOK_MEM_ALLOC],
                   "Failed to find symbol cuMemAlloc, %s\n", dlerror());

    realFunctions[CU_HOOK_MEM_FREE] =
        real_dlsym(handle, CUDA_SYMBOL_STRING(cuMemFree));
    CU_HOOK_DIE_IF(!realFunctions[CU_HOOK_MEM_FREE],
                   "Failed to find symbol cuMemFree, %s\n", dlerror());

    realFunctions[CU_HOOK_CTX_GET_CURRENT] =
        real_dlsym(handle, CUDA_SYMBOL_STRING(cuCtxGetCurrent));
    CU_HOOK_DIE_IF(!realFunctions[CU_HOOK_CTX_GET_CURRENT],
                   "Failed to find symbol cuCtxGetCurrent, %s\n", dlerror());

    realFunctions[CU_HOOK_CTX_SET_CURRENT] =
        real_dlsym(handle, CUDA_SYMBOL_STRING(cuCtxSetCurrent));
    CU_HOOK_DIE_IF(!realFunctions[CU_HOOK_CTX_SET_CURRENT],
                   "Failed to find symbol cuCtxSetCurrent, %s\n", dlerror());

    realFunctions[CU_HOOK_CTX_DESTROY] =
        real_dlsym(handle, CUDA_SYMBOL_STRING(cuCtxDestroy));
    CU_HOOK_DIE_IF(!realFunctions[CU_HOOK_CTX_DESTROY],
                   "Failed to find symbol cuCtxDestroy, %s\n", dlerror());
...
#define CU_HOOK_GENERATE_INTERCEPT(hooksymbol, funcname, params, ...)          \
  CUresult CUDAAPI funcname params {                                           \
    ...
    result = ((CUresult CUDAAPI(*) params)cuhl.realFunctions[hooksymbol])(     \
        __VA_ARGS__);                                                          \
