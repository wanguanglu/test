...
// This sample needs at least CUDA 5.5 and a GPU that has at least Compute
// Capability 2.0

// This sample demonstrates a simple image processing pipeline.
// First, a JPEG file is huffman decoded and inverse DCT transformed and
// dequantized. Then the different planes are resized. Finally, the resized
// image is quantized, forward DCT transformed and huffman encoded.
...
void writeJFIFTag(unsigned char *&pData) {
  ...
  writeAndAdvance<unsigned short>(pData,
                                  sizeof(JFIF_TAG) + sizeof(unsigned short));
...
void readQuantizationTables(const unsigned char *pData,
  ...
  while (nLength > 0) {
    unsigned char nPrecisionAndIdentifier =
        readAndAdvance<unsigned char>(pData);
...
void printHelp() {
  ...
  cout << "   -scale=1.0             (scale multiplier for width and height)"
       << endl
       << endl;
}
...
int main(int argc, char **argv) {
  ...
  while (nMarker != -1) {
    ...
    if ((nMarker == 0x0C0) | (nMarker == 0x0C2)) {
      ...
      cout << "Image Size: " << oFrameHeader.nWidth << "x"
           << oFrameHeader.nHeight << "x"
           << static_cast<int>(oFrameHeader.nComponents) << endl;
      ...
      // Compute channel sizes as stored in the JPEG (8x8 blocks & MCU block
      // layout)
      ...
      for (int i = 0; i < oFrameHeader.nComponents; ++i) {
        ...
        oBlocks.width =
            (int)ceil((oFrameHeader.nWidth + 7) / 8 *
                      static_cast<float>(oBlocksPerMCU.width) / nMCUBlocksH);
        oBlocks.width =
            DivUp(oBlocks.width, oBlocksPerMCU.width) * oBlocksPerMCU.width;

        oBlocks.height =
            (int)ceil((oFrameHeader.nHeight + 7) / 8 *
                      static_cast<float>(oBlocksPerMCU.height) / nMCUBlocksV);
        oBlocks.height =
            DivUp(oBlocks.height, oBlocksPerMCU.height) * oBlocksPerMCU.height;
        ...
        // Allocate Memory
        size_t nPitch;
        NPP_CHECK_CUDA(cudaMallocPitch(&apdDCT[i], &nPitch,
                                       oBlocks.width * 64 * sizeof(Npp16s),
                                       oBlocks.height));
        aDCTStep[i] = static_cast<Npp32s>(nPitch);

        NPP_CHECK_CUDA(cudaMallocPitch(&apSrcImage[i], &nPitch,
                                       aSrcSize[i].width, aSrcSize[i].height));
        aSrcImageStep[i] = static_cast<Npp32s>(nPitch);

        NPP_CHECK_CUDA(cudaHostAlloc(&aphDCT[i], aDCTStep[i] * oBlocks.height,
    ...
    if (nMarker == 0x0DA) {
      ...
      int nAfterScanMarker =
          nextMarker(pJpegData, nAfterNextMarkerPos, nInputLength);
      ...
      for (int i = 0; i < 3; ++i) {
        nppiDecodeHuffmanSpecInitAllocHost_JPEG(
            pHuffmanDCTables[(oScanHeader.aHuffmanTablesSelector[i] >> 4)]
                .aCodes,
            nppiDCTable, &apHuffmanDCTable[i]);
        nppiDecodeHuffmanSpecInitAllocHost_JPEG(
            pHuffmanACTables[(oScanHeader.aHuffmanTablesSelector[i] & 0x0f)]
                .aCodes,
            nppiACTable, &apHuffmanACTable[i]);
      }

      NPP_CHECK_NPP(nppiDecodeHuffmanScanHost_JPEG_8u16s_P3R(
          pJpegData + nPos, nAfterNextMarkerPos - nPos - 2, nRestartInterval,
          oScanHeader.nSs, oScanHeader.nSe, oScanHeader.nA >> 4,
          oScanHeader.nA & 0x0f, aphDCT, aDCTStep, apHuffmanDCTable,
          apHuffmanACTable, aSrcSize));
  ...
  for (int i = 0; i < 4; ++i) {
    NPP_CHECK_CUDA(cudaMemcpyAsync(pdQuantizationTables + i * 64,
                                   aQuantizationTables[i].aTable, 64,
                                   cudaMemcpyHostToDevice));
  }

  for (int i = 0; i < 3; ++i) {
    NPP_CHECK_CUDA(cudaMemcpyAsync(apdDCT[i], aphDCT[i],
  ...
  // Inverse DCT
  for (int i = 0; i < 3; ++i) {
    NPP_CHECK_NPP(nppiDCTQuantInv8x8LS_JPEG_16s8u_C1R_NEW(
        apdDCT[i], aDCTStep[i], apSrcImage[i], aSrcImageStep[i],
        pdQuantizationTables + oFrameHeader.aQuantizationTableSelector[i] * 64,
        aSrcSize[i], pDCTState));
  }
  ...
  // Compute channel sizes as stored in the output JPEG (8x8 blocks & MCU block
  // layout)
  ...
  cout << "Output Size: " << oDstImageSize.width << "x" << oDstImageSize.height
       << "x" << static_cast<int>(oFrameHeader.nComponents) << endl;

  for (int i = 0; i < oFrameHeader.nComponents; ++i) {
    NppiSize oBlocks;
    NppiSize oBlocksPerMCU = {oFrameHeader.aSamplingFactors[i] & 0x0f,
                              oFrameHeader.aSamplingFactors[i] >> 4};

    oBlocks.width =
        (int)ceil((oDstImageSize.width + 7) / 8 *
                  static_cast<float>(oBlocksPerMCU.width) / nMCUBlocksH);
    oBlocks.width =
        DivUp(oBlocks.width, oBlocksPerMCU.width) * oBlocksPerMCU.width;

    oBlocks.height =
        (int)ceil((oDstImageSize.height + 7) / 8 *
                  static_cast<float>(oBlocksPerMCU.height) / nMCUBlocksV);
    oBlocks.height =
        DivUp(oBlocks.height, oBlocksPerMCU.height) * oBlocksPerMCU.height;
    ...
    // Allocate Memory
    size_t nPitch;
    NPP_CHECK_CUDA(cudaMallocPitch(&apDstImage[i], &nPitch, aDstSize[i].width,
                                   aDstSize[i].height));
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    NppiSize oSrcImageSize = {
        (oFrameHeader.nWidth * oBlocksPerMCU.width) / nMCUBlocksH,
        (oFrameHeader.nHeight * oBlocksPerMCU.height) / nMCUBlocksV};
    ...
    NPP_CHECK_NPP(nppiResizeSqrPixel_8u_C1R(
        apSrcImage[i], oSrcImageSize, aSrcImageStep[i], oSrcImageROI,
        apDstImage[i], aDstImageStep[i], oDstImageROI, nScaleFactor,
        nScaleFactor, 0.5, 0.5, eInterploationMode));
  }
  ...
  for (int i = 0; i < 3; ++i) {
    NPP_CHECK_NPP(nppiDCTQuantFwd8x8LS_JPEG_8u16s_C1R_NEW(
        apDstImage[i], aDstImageStep[i], apdDCT[i], aDCTStep[i],
        pdQuantizationTables + oFrameHeader.aQuantizationTableSelector[i] * 64,
  ...
  NPP_CHECK_NPP(nppiEncodeHuffmanScan_JPEG_8u16s_P3R(
      apdDCT, aDCTStep, 0, oScanHeader.nSs, oScanHeader.nSe,
      oScanHeader.nA >> 4, oScanHeader.nA & 0x0f, pdScan, &nScanLength,
      apHuffmanDCTable, apHuffmanACTable, aDstSize, pJpegEncoderTemp));
  ...
  NPP_CHECK_CUDA(
      cudaMemcpy(pDstOutput, pdScan, nScanLength, cudaMemcpyDeviceToHost));
