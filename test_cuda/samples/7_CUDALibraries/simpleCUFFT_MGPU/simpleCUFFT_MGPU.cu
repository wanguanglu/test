...
int main(int argc, char** argv) {
  ...
  result = cufftXtMemcpy(plan_input, d_signal, h_padded_signal,
                         CUFFT_COPY_HOST_TO_DEVICE);
  if (result != CUFFT_SUCCESS) {
    printf("*XtMemcpy failed\n");
    exit(EXIT_FAILURE);
  }
  result = cufftXtMemcpy(plan_input, d_filter_kernel, h_padded_filter_kernel,
                         CUFFT_COPY_HOST_TO_DEVICE);
  if (result != CUFFT_SUCCESS) {
    printf("*XtMemcpy failed\n");
    exit(EXIT_FAILURE);
  ...
  result = cufftXtExecDescriptorC2C(plan_input, d_filter_kernel,
  ...
  result = cufftXtMemcpy(plan_input, d_out_signal, d_signal,
                         CUFFT_COPY_DEVICE_TO_DEVICE);
  if (result != CUFFT_SUCCESS) {
    printf("*XtMemcpy failed\n");
    exit(EXIT_FAILURE);
  }
  result = cufftXtMemcpy(plan_input, d_out_filter_kernel, d_filter_kernel,
                         CUFFT_COPY_DEVICE_TO_DEVICE);
  if (result != CUFFT_SUCCESS) {
    printf("*XtMemcpy failed\n");
    exit(EXIT_FAILURE);
  ...
  result = cufftXtExecDescriptorC2C(plan_input, d_out_signal, d_out_signal,
                                    CUFFT_INVERSE);
  if (result != CUFFT_SUCCESS) {
    printf("*XtExecC2C  failed\n");
    exit(EXIT_FAILURE);
