  ...
  if (0 == pid) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (prop.computeMode != cudaComputeModeDefault) {
        ...
        printf(
            "> Please use nvidia-smi to change the Compute Mode to Default\n");
        exit(EXIT_SUCCESS);
      }
    }
    ...
    for (i = 1; i < uvaCount; i++) {
      checkCudaErrors(cudaDeviceCanAccessPeer(&canAccessPeer_0i, uvaOrdinals[0],
                                              uvaOrdinals[i]));
      checkCudaErrors(cudaDeviceCanAccessPeer(&canAccessPeer_i0, uvaOrdinals[i],
                                              uvaOrdinals[0]));
...
void runTestMultiKernel(ipcCUDA_t *s_mem, int index) {
  ...
  if (index == 0) {
    ...
    checkCudaErrors(
        cudaMemcpy(h_results, d_ptr + DATA_BUF_SIZE,
    ...
    for (int n = 1; n < g_processCount; n++) {
      for (int i = 0; i < DATA_BUF_SIZE; i++) {
        if (h_refData[i] / (n + 1) != h_results[(n - 1) * DATA_BUF_SIZE + i]) {
          fprintf(stderr,
                  "Data check error at index %d in process %d!: %i,    %i\n", i,
...
int main(int argc, char **argv) {
  ...
  // distribute UVA capable devices among processes (1 device per
  // PROCESSES_PER_DEVICE processes) if there is only one device, have 1 extra
  // process
  if (s_devices->count > 1) {
