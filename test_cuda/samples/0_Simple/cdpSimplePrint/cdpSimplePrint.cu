...
int main(int argc, char **argv) {
  ...
  if (checkCmdLineFlag(argc, (const char **)argv, "device")) {
    ...
    if (properties.major > 3 ||
      ...
      std::cout << "Running on GPU " << device << " (" << properties.name << ")"
                << std::endl;
    } else {
      std::cout << "ERROR: cdpsimplePrint requires GPU devices with compute SM "
                   "3.5 or higher."
                << std::endl;
      std::cout << "Current GPU device has compute SM" << properties.major
                << "." << properties.minor << ". Exiting..." << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  if (device == -1) {
    std::cerr << "cdpSimplePrint requires GPU devices with compute SM 3.5 or "
                 "higher.  Exiting..."
              << std::endl;
    exit(EXIT_WAIVED);
  }
