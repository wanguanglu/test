...
__global__ void shiftPitchLinear(float *odata, int pitch, int width, int height,
  ...
  odata[yid * pitch + xid] = tex2D(texRefPL, (xid + shiftX) / (float)width,
...
__global__ void shiftArray(float *odata, int pitch, int width, int height,
  ...
  odata[yid * pitch + xid] = tex2D(texRefArray, (xid + shiftX) / (float)width,
                                   (yid + shiftY) / (float)height);
}
...
void runTest(int argc, char **argv) {
  ...
  checkCudaErrors(cudaBindTexture2D(0, &texRefPL, d_idataPL, &channelDesc, nx,
                                    ny, d_pitchBytes));
  ...
  for (int i = 0; i < NUM_REPS; ++i) {
    shiftPitchLinear<<<dimGrid, dimBlock>>>(
        d_odata, (int)(d_pitchBytes / sizeof(float)), nx, ny, x_shift, y_shift);
  }
