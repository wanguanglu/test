...
static CUresult initCUDA(int argc, char** argv, CUfunction* pMatrixMul,
  ...
  *block_size_out = block_size;
  ...
  printf("  64-bit Memory Address:             %s\n",
         (totalGlobalMem > (unsigned long long)4 * 1024 * 1024 * 1024L) ? "YES"
                                                                        : "NO");
  ...
  if (totalGlobalMem > (unsigned long long)4 * 1024 * 1024 * 1024L) {
    if (block_size == 16) {
      status =
          cuModuleGetFunction(&cuFunction, cuModule, "matrixMul_bs16_64bit");
    } else {
      status =
          cuModuleGetFunction(&cuFunction, cuModule, "matrixMul_bs32_64bit");
    }
  } else
#endif
  {
    if (block_size == 16) {
      status =
          cuModuleGetFunction(&cuFunction, cuModule, "matrixMul_bs16_32bit");
    } else {
      status =
          cuModuleGetFunction(&cuFunction, cuModule, "matrixMul_bs32_32bit");
    }
  }
