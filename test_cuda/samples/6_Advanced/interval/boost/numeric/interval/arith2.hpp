...
inline interval<T, Policies> division_part1(const interval<T, Policies>& x,
  ...
  b = false;
  if (detail::test_input(x, y)) return I::empty();
  if (zero_in(y))
...
template <class T, class Policies>
inline interval<T, Policies> multiplicative_inverse(
    const interval<T, Policies>& x) {
  typedef interval<T, Policies> I;
  if (detail::test_input(x)) return I::empty();
...
inline interval<T, Policies> pow(const interval<T, Policies>& x, int pwr) {
  ...
  if (interval_lib::user::is_neg(x.upper())) {  // [-2,-1]
    ...
    if (pwr & 1) {                                     // [-1,1]^1
      return I(-pow_up(-x.lower(), pwr, rnd), pow_up(x.upper(), pwr, rnd),
               true);
    } else {  // [-1,1]^2
      return I(static_cast<T>(0),
               pow_up(max BOOST_PREVENT_MACRO_SUBSTITUTION(
                          static_cast<T>(-x.lower()), x.upper()),
                      pwr, rnd),
               true);
    }
  } else {  // [1,2]
...
inline interval<T, Policies> sqrt(const interval<T, Policies>& x) {
  ...
  if (interval_lib::detail::test_input(x) ||
    ...
    return I::empty();
  typename Policies::rounding rnd;
  T l = !interval_lib::user::is_pos(x.lower()) ? static_cast<T>(0)
                                               : rnd.sqrt_down(x.lower());
...
inline interval<T, Policies> square(const interval<T, Policies>& x) {
  ...
  if (interval_lib::user::is_neg(xu))
    ...
    return I(static_cast<T>(0),
             (-xl > xu ? rnd.mul_up(xl, xl) : rnd.mul_up(xu, xu)), true);
}
...
template <class I>
inline I root_aux(typename I::base_type const& x,
                  int k)  // x and k are bigger than one
{
  ...
  for (;;) {
    T y0 = median(y);
    I yy =
        intersect(y, y0 - (pow(I(y0, y0, true), k) - x) / (tk * pow(y, k - 1)));
...
inline  // x is positive and k bigger than one
    ...
    root_aux_dn(typename I::base_type const& x, int k) {
  typedef typename I::base_type T;
...
inline  // x is positive and k bigger than one
    typename I::base_type
    root_aux_up(typename I::base_type const& x, int k) {
  typedef typename I::base_type T;
  typedef typename I::traits_type Policies;
  typename Policies::rounding rnd;
  T one(1);
...
inline interval<T, Policies> nth_root(interval<T, Policies> const& x, int k) {
  ...
  if (!interval_lib::user::is_pos(x.upper())) {
    if (interval_lib::user::is_zero(x.upper())) {
      T zero(0);
      if (!(k & 1) ||
          interval_lib::user::is_zero(x.lower()))  // [-1,0]^/2 or [0,0]
        return I(zero, zero, true);
      else  // [-1,0]^/3
        return I(-interval_lib::detail::root_aux_up<R>(-x.lower(), k), zero,
                 true);
    } else if (!(k & 1))  // [-2,-1]^/2
      return I::empty();
    else {  // [-2,-1]^/3
  ...
  if (!interval_lib::user::is_pos(x.lower()))
    if (!(k & 1) ||
        interval_lib::user::is_zero(x.lower()))  // [-1,1]^/2 or [0,1]
