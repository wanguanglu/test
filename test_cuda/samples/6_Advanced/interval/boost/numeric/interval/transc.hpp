...
inline interval<T, Policies> log(const interval<T, Policies>& x) {
  ...
  T l = !interval_lib::user::is_pos(x.lower()) ? checking::neg_inf()
...
inline interval<T, Policies> cos(const interval<T, Policies>& x) {
  ...
  if (u <= interval_lib::constants::pi_lower<T>())
    ...
    return I(static_cast<T>(-1),
             rnd.cos_up(min BOOST_PREVENT_MACRO_SUBSTITUTION(
                 rnd.sub_down(pi2.lower(), u), l)),
             true);
  else
...
inline interval<T, Policies> acos(const interval<T, Policies>& x) {
  ...
  if (interval_lib::detail::test_input(x) || x.upper() < static_cast<T>(-1) ||
    ...
    return I::empty();
  typename Policies::rounding rnd;
  T l = (x.upper() >= static_cast<T>(1)) ? static_cast<T>(0)
...
inline interval<T, Policies> cosh(const interval<T, Policies>& x) {
  ...
  if (interval_lib::user::is_neg(x.upper()))
    ...
    return I(static_cast<T>(0),
             rnd.cosh_up(-x.lower() > x.upper() ? x.lower() : x.upper()), true);
}
...
inline interval<T, Policies> acosh(const interval<T, Policies>& x) {
  ...
  if (interval_lib::detail::test_input(x) || x.upper() < static_cast<T>(1))
    return I::empty();
  typename Policies::rounding rnd;
  T l = x.lower() <= static_cast<T>(1) ? static_cast<T>(0)
                                       : rnd.acosh_down(x.lower());
...
inline interval<T, Policies> atanh(const interval<T, Policies>& x) {
  ...
  typename Policies::rounding rnd;
  typedef typename Policies::checking checking;
  T l = (x.lower() <= static_cast<T>(-1)) ? checking::neg_inf()
                                          : rnd.atanh_down(x.lower());
  T u = (x.upper() >= static_cast<T>(1)) ? checking::pos_inf()
