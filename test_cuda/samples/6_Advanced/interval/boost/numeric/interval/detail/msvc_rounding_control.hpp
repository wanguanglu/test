...
// This is updated as per fix in boost 1.58
                   ...
                   // x64 ignores _MCW_PC and _MCW_IC, and the Debug CRT library
                   // actually asserts when these are passed to _control87. MSDN
                   // says on '_control87' that changing precision (_MCW_PC) or
                   // infinity (_MCW_IC) handling is not supported on the ARM
                   // and x64 architectures and that _control87 raises an
                   // assertion and the invalid parameter handler is invoked.
                   | _MCW_PC | _MCW_IC
#endif
    );
  }
