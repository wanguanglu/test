  ...
  *info = 0;
  ...
  for (int j = 0; j < minDim; j += nb) {
    ...
    *info = s_info;
    ...
    // We update ipiv in parallel on the device, if we were launched with >1
    // threads
    for (int i = j + threadIdx.x, end = min(m, j + jb); i < end;
         i += blockDim.x)
      ipiv[i] += j;
    ...
    if (threadIdx.x == 0) {
      ...
      if (j + jb < n) {
        dlaswp<<<1, 256, 0, stream>>>(n - j - jb, &A[(j + jb) * lda], lda, ipiv,
                                      j, j + jb);

        double one = 1.0;
        status =
            cublasDtrsm_v2(cb_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER,
                           CUBLAS_OP_N, CUBLAS_DIAG_UNIT, jb, n - j - jb, &one,
    ...
    if (threadIdx.x == 0 && j + jb < m) {
      ...
      status = cublasDgemm_v2(cb_handle, CUBLAS_OP_N, CUBLAS_OP_N, m - j - jb,
                              n - j - jb, jb, &minus_one, &A[j * lda + j + jb],
                              lda, &A[(j + jb) * lda + j], lda, &one,
                              &A[(j + jb) * lda + j + jb], lda);
...
__global__ void dgetrf_cdpentry(Parameters *device_params) {
  ...
  dgetrf(cb_handle, stream, device_params->m, device_params->n,
         ...
         device_params->device_piv, device_params->device_info);
}
