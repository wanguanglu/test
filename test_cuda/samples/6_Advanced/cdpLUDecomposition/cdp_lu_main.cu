...
void memsetup(Parameters &host_params) {
  ...
  checkCudaErrors(cudaMemcpy(host_params.device_LU, host_params.host_LU, len,
                             cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(host_params.device_piv, host_params.host_piv,
                             piv_len, cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(host_params.device_info, &host_params.device_info,
...
bool checkresult(Parameters &host_params) {
  ...
  status = cublasDtrmm_v2(cb_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER,
                          CUBLAS_OP_N, CUBLAS_DIAG_UNIT, host_params.m,
  ...
  checkCudaErrors(cudaMemcpy(host_params.host_LU, host_params.device_LU, len,
                             cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(host_params.host_piv, host_params.device_piv,
                             piv_len, cudaMemcpyDeviceToHost));
  ...
  // Verify that L*U = A.
  checkCudaErrors(
      cudaMemcpy(host_result, device_result, len, cudaMemcpyDeviceToHost));
  bool ok = true;

  for (int i = 0; i < host_params.m; i++)
    for (int j = 0; j < host_params.n; j++)
      if (fabs(host_result[lda * j + i] -
               host_params.host_A[j * lda + perm[i]]) > tol) {
        printf("(%d,%d): found=%f, expected=%f\n", i, j,
               host_result[lda * j + i], host_params.host_A[j * lda + perm[i]]);
        ok = false;
        break;
      }
