...
//  shared memory).
//
//  We handle non-power-of-2 sizes by padding out to the next largest power
//  of 2. This is the fully-generic version, for sorting arbitrary data which
//  does not have a clear "maximum" value. We track "invalid" entries in a
//  separate array to make sure that they always sorts as "max value" elements.
//  A template parameter "OOR" allows specialisation to optimise away the
//  invalid tracking.
//
...
static __device__ __forceinline__ void bitonicsort_kernel(unsigned *indata,
  ...
  // style sort) and "j" is the partner element in the butterfly. Two threads
