...
//  inter-warp communication. This has a high atomic issue rate, but the rest
//  should actually be fairly quick because of low work per thread.
//
...
__global__ void qsort_warp(unsigned* indata, unsigned* outdata,
  ...
  if (lane_id == 0) {
    ...
    if (atomicAdd((unsigned int*)&atomicData->sorted_count, mycount) +
      ...
      // algorithm if we free when we're done, rather than re-using (makes for
      // less fragmentation).
