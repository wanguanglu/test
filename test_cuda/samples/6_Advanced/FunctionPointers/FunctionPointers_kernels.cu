...
// pixel value used for thresholding function, works well with sample image
// 'lena'
...
void setupFunctionTables() {
  ...
  // an index into the table to choose them We have now set the order in the
  // function table according to our enum.
  checkCudaErrors(
...
// shown. BlockOperation is an integer kernel argument used as an index into the
