 ...
 *be associated with a CPU process.  CUDA Contexts have a one-to-one
 *correspondence with host threads.  A host thread may have only one device
...
static CUresult InitCUDAContext(CUDAContext *pContext, CUdevice hcuDevice,
  ...
  if (!findModulePath(PTX_FILE, module_path, argv, ptx_source)) {
    if (!findModulePath(CUBIN_FILE, module_path, argv, ptx_source)) {
      fprintf(
          stderr,
          "> findModulePath could not find <threadMigration> ptx or cubin\n");
      CLEANUP_ON_ERROR(dptr, hcuModule, hcuContext, status);
    }
  }
  ...
  if (ThreadLaunchCount != NumThreads * deviceCount) {
    ...
    for (int iDevice = 0; iDevice < deviceCount; iDevice++) {
      for (int iThread = 0; iThread < NumThreads; iThread++, ThreadIndex++) {
        ...
        if (CUDA_SUCCESS !=
            cuCtxDestroy(g_ThreadParams[ThreadIndex].hcuContext))
          return false;
      }
    }
