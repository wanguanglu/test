...
struct Graph {
  ...
  Graph(uint verticesCount, uint edgesCount)
      : vertices(verticesCount), edges(edgesCount), weights(edgesCount) {}
...
public:
  ...
  void dump(uint width, uint height) const {
    ...
    uint requiredDigitsCount =
        static_cast<uint>(log10(static_cast<float>(levelsCount()))) + 1;
...
private:
  ...
  public:
    ...
    void buildFromDeviceData(thrust::device_ptr<uint> superVerticesOffsets,
                             thrust::device_ptr<uint> verticesIDs) {
      checkCudaErrors(cudaMemcpy(
          &(superNodesOffsets_[0]), superVerticesOffsets.get(),
          sizeof(uint) * superNodesOffsets_.size(), cudaMemcpyDeviceToHost));

      checkCudaErrors(cudaMemcpy(&(nodes_[0]), verticesIDs.get(),
      ...
      for (uint superNodeIndex = 0, nodeIndex = 0;
        ...
        for (; nodeIndex < superNodeEnd; ++nodeIndex) {
          nodesQueue.push_back(
              std::make_pair(nodes[nodeIndex], superNodeIndex));
        }
      }
    }
...
public:
  ...
  float run(const Graph &graph, Pyramid &segmentations) {
    ...
    MemoryPoolsCollection pools = {
        DeviceMemoryPool<uint>(static_cast<uint>(graph.vertices.size()),
                               kUintVerticesPoolsRequired),
        DeviceMemoryPool<float>(static_cast<uint>(graph.vertices.size()),
                                kFloatVerticesPoolsRequired),
        DeviceMemoryPool<uint>(static_cast<uint>(graph.edges.size()),
                               kUintEdgesPoolsRequired),
        DeviceMemoryPool<float>(static_cast<uint>(graph.edges.size()),
...
private:
  void printMemoryUsage() {
    ...
    cout << "Device memory: used " << usedMemory << " available "
         << availableMemory << " total " << totalMemory << endl;
  }
  ...
  void initalizeData(const Graph &graph, MemoryPoolsCollection &pools) {
    ...
    checkCudaErrors(cudaMemcpy(dVertices_.get(), &(graph.vertices[0]),
                               sizeof(uint) * verticesCount_,
                               cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(dEdges_.get(), &(graph.edges[0]),
                               sizeof(uint) * edgesCount_,
                               cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(dWeights_.get(), &(graph.weights[0]),
  ...
  AlgorithmStatus invokeStep(MemoryPoolsCollection &pools,
    ...
    calculateThreadsDistribution(verticesCount_, blocksCount,
                                 threadsPerBlockCount);
    ...
    getSuccessors<<<gridDimsForVertices, blockDimsForVertices, 0>>>(
        dVertices_.get(), dMinScannedEdges.get(), dSuccessors.get(),
        verticesCount_, edgesCount_);
    ...
    thrust::inclusive_scan(dEdgesFlags, dEdgesFlags + edgesCount_,
                           dStartpoints);

    addScalar<<<gridDimsForEdges, blockDimsForEdges, 0>>>(dStartpoints.get(),
                                                          -1, edgesCount_);
    ...
    thrust::sort(
        ...
        thrust::make_zip_iterator(thrust::make_tuple(
            thrust::device_ptr<uint>(dSuccessors + verticesCount_),
    ...
    thrust::inclusive_scan(dVerticesFlags_, dVerticesFlags_ + verticesCount_,
                           dNewVerticesIDs_);
    ...
    cudaMemcpy(&newVerticesCount, (dNewVerticesIDs_ + verticesCount_ - 1).get(),
    ...
    if (newVerticesCount == verticesCount_) {
      ...
      segmentations.addLevel(1, verticesCount_, dDummyVerticesOffsets,
                             dDummyVerticesIDs);
    ...
    getVerticesMapping<<<gridDimsForVertices, blockDimsForVertices, 0>>>(
        dClusteredVerticesIDs.get(), dNewVerticesIDs_.get(),
        dVerticesMapping.get(), verticesCount_);
    ...
    calculateEdgesInfo<<<gridDimsForEdges, blockDimsForEdges, 0>>>(
        dStartpoints.get(), dVerticesMapping.get(), dEdges_.get(),
        dWeights_.get(), dNewStartpoints.get(), dSurvivedEdgesIDs.get(),
        edgesCount_, newVerticesCount);
    ...
    thrust::sort(
        thrust::make_zip_iterator(
            thrust::make_tuple(dNewStartpoints, dSurvivedEdgesIDs)),
        thrust::make_zip_iterator(thrust::make_tuple(
    ...
    thrust::copy_if(thrust::make_counting_iterator(0U),
                    ...
                    dEdgesFlags, dVertices_, thrust::identity<uint>())
        .get();
    ...
    calculateThreadsDistribution(validEdgesCount, blocksCount,
                                 threadsPerBlockCount);
    ...
    makeNewEdges<<<newGridDimsForEdges, newBlockDimsForEdges, 0>>>(
        dSurvivedEdgesIDs.get(), dVerticesMapping.get(), dEdges_.get(),
        dWeights_.get(), dNewEdges.get(), dNewWeights.get(), validEdgesCount);
    ...
    thrust::sort_by_key(dVerticesMapping, dVerticesMapping + verticesCount_,
                        dVerticesIDs);
    ...
    segmentations.addLevel(newVerticesCount, verticesCount_,
                           dNewVerticesOffsets, dVerticesIDs);
...
int main(int argc, char **argv) {
  ...
  bResults[0] =
      sdkComparePPM("level_00.ppm", sdkFindFilePath("ref_00.ppm", argv[0]),
                    5.0f, 0.15f, false);
  bResults[1] =
      sdkComparePPM("level_09.ppm", sdkFindFilePath("ref_09.ppm", argv[0]),
                    5.0f, 0.15f, false);
