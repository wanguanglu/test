...
extern "C" void initTexture(int imageWidth, int imageHeight, uchar *h_data) {
  ...
  checkCudaErrors(cudaMemcpyToArray(d_imageArray, 0, 0, h_data, size,
...
extern "C" void freeTexture() { checkCudaErrors(cudaFreeArray(d_imageArray)); }
...
extern "C" void render(int width, int height, float tx, float ty, float scale,
  ...
  case MODE_NEAREST:
    ...
    d_render<<<gridSize, blockSize>>>(output, width, height, tx, ty, scale, cx,
                                      cy);
    break;
  ...
  case MODE_FAST_BICUBIC:
    tex.filterMode = cudaFilterModeLinear;
    d_renderFastBicubic<<<gridSize, blockSize>>>(output, width, height, tx, ty,
                                                 scale, cx, cy);
    break;
