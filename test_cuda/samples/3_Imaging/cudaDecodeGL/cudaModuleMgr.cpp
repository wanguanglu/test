...
//      nKernels        - total # of different CUDA kernel functions in the
//      CUBIN/OTX nGlobalMem      - total # of Global Memory arrays defined in
//      the CUBIN/PTX (i.e. constants) nTexRef         - total # of Texture
//      References arrays defined in the CUBIN/PTX (i.e. texture arrays)
extern "C" bool modInitCTX(sCtxModule* pCtx, const char* filename,
  ...
  if (pCtx->mModuleName.rfind(".ptx") != string::npos) {
    ...
    cuStatus =
        cuModuleLoadDataEx(&pCtx->cuModule_, ptx_source.c_str(), jitNumOptions,
  ...
  printf(">> modInitCTX<%20s > initialized %s", pCtx->mModuleName.c_str(),
...
extern "C" CUresult modGetCudaFunction(sCtxModule* pCtx, const char* func_name,
  ...
  if (fpCudaKernel) *fpCudaKernel = pCtx->pCudaKernels_[idx].fpCuda;
...
extern "C" CUresult modGetCudaDevicePtr(sCtxModule* pCtx,
  ...
  if (pGlobalMem) *pGlobalMem = pCtx->pGlobalMem_[idx].devicePtr;
...
extern "C" CUresult modGetTexRef(sCtxModule* pCtx, const char* texref_name,
                                 CUtexref* pTexRef) {
  int idx = pCtx->nLastTexRef_;

  CUresult cuStatus = cuModuleGetTexRef(&(pCtx->pTexRef_[idx].texRef),
                                        pCtx->cuModule_, texref_name);
  ...
  if (pTexRef) *pTexRef = pCtx->pTexRef_[idx].texRef;
...
extern "C" int modFindIndex_CudaKernels(sCtxModule* pCtx,
...
extern "C" int modFindIndex_GlobalMem(sCtxModule* pCtx,
                                      const char* address_name) {
  int found = -1;
...
extern "C" CUresult modLaunchKernel(sCtxModule* pCtx, CUfunction fpFunc,
                                    dim3 block, dim3 grid) {
  CUresult error = CUDA_SUCCESS;
  /*
      dim3 block(32,16);
      dim3 grid((width+(2*block.x-1))/(2*block.x),
     ...
     += sizeof(d_srcNV12); cutilDrvSafeCall(cuParamSeti        ( fpFunc, 4,
     nSourcePitch ));  offset += sizeof(nSourcePitch);
      cutilDrvSafeCall(cuParamSeti        ( fpFunc, 8,  d_dstARGB ));     offset
     += sizeof(d_dstARGB); cutilDrvSafeCall(cuParamSeti        ( fpFunc, 12,
     nDestPitch ));    offset += sizeof(nDestPitch);
      cutilDrvSafeCall(cuParamSeti        ( fpFunc, 16, width ));         offset
     += sizeof(width); cutilDrvSafeCall(cuParamSeti        ( fpFunc, 20, height
     ));        offset += sizeof(height); cutilDrvSafeCall(cuParamSetSize     (
     fpFunc,     offset));
...
CUmoduleManager::CUmoduleManager(const char* filename_module,
                                 const char* exec_path, int nKernels,
                                 int nGlobalMem, int nTexRef) {
  if (modInitCTX(&mCtx, filename_module, exec_path, nKernels, nGlobalMem,
                 nTexRef) == false) {
    throw(filename_module);
  }
}

CUmoduleManager::~CUmoduleManager() { modFreeCTX(&mCtx); }

// This gets the CUDA kernel function pointers from the CUBIN/PTX
CUresult CUmoduleManager::GetCudaFunction(const char* func_name,
                                          CUfunction* fpCudaKernel) {
  return modGetCudaFunction(&mCtx, func_name, fpCudaKernel);
}

// This gets the CUDA device pointers from the CUBIN/PTX
CUresult CUmoduleManager::GetCudaDevicePtr(const char* address_name,
                                           CUdeviceptr* pGlobalMem) {
  return modGetCudaDevicePtr(&mCtx, address_name, pGlobalMem);
}

// This retrieves the CUDA Texture References from the CUBIN/PTX
CUresult CUmoduleManager::GetTexRef(const char* texref_name,
                                    CUtexref* pTexRef) {
...
CUresult CUmoduleManager::launchKernel(CUfunction fpFunc, dim3 block,
                                       dim3 grid) {
