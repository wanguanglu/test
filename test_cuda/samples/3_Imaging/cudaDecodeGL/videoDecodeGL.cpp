...
StopWatchInterface *frame_timer = NULL, *global_timer = NULL;
...
bool g_bIsProgressive =
    true; // assume it is progressive, unless otherwise noted
...
bool loadVideoSource(const char *video_file, unsigned int &width,
                     unsigned int &height, unsigned int &dispWidth,
                     unsigned int &dispHeight);
void initCudaVideo();

void freeCudaResources(bool bDestroyContext);

bool copyDecodedFrameToTexture(unsigned int &nRepeats, int bUseInterop,
                               int *pbIsProgressive);
void cudaPostProcessFrame(CUdeviceptr *ppDecodedFrame, size_t nDecodedPitch,
                          CUdeviceptr *ppInteropFrame, size_t pFramePitch,
                          CUmodule cuModNV12toARGB, CUfunction fpCudaKernel,
                          CUstream streamID);
...
void setVSync(int interval) {
  ...
  if (strstr((const char *)extensions, "WGL_EXT_swap_control") == 0) {
    return; // Error: WGL_EXT_swap_control extension not supported on your
            // computer.\n");
  } else {
    wglSwapIntervalEXT =
...
void computeFPS(int bUseInterop) {
  ...
  if (g_bDeviceLost) {
    ...
    sprintf(sFPS, "%s [%s] - [%s %d]", sSDKname, sDecodeStatus.c_str(),
  ...
  if (!g_pFrameQueue->isDecodeFinished() && !g_bRunning) {
    ...
    sprintf(sFPS, "%s [%s] - [%s %d] - Video Display %s / Vsync %s",
            ...
            (g_bIsProgressive ? "Frame" : "Field"), g_DecodeFrameCount,
            g_bUseDisplay ? "ON" : "OFF", g_bUseVsync ? "ON" : "OFF");
  ...
  if (g_fpsCount == g_fpsLimit) {
    ...
    printf("[%s] - [%s: %04d, %04.1f fps, frame time: %04.2f (ms) ]\n",
           sAppFilename, (g_bIsProgressive ? "Frame" : "Field"), g_FrameCount,
           ifps, 1000.f / ifps);
...
bool initCudaResources(int argc, char **argv, int *bTCC) {
  ...
  // Device is specified at the command line, we need to check if this it TCC or
  // not, and then call the appropriate TCC/non-TCC findCudaDevice in order to
  ...
  // Create CUDA Device w/ GL interop (if not TCC), otherwise CUDA w/o interop
  // (if TCC) (use CU_CTX_BLOCKING_SYNC for better CPU synchronization)
  if (g_bUseInterop && !(*bTCC)) {
    checkCudaErrors(
        cuGLCtxCreate(&g_oContext, CU_CTX_BLOCKING_SYNC, g_oDevice));
  } else {
    checkCudaErrors(cuCtxCreate(&g_oContext, CU_CTX_BLOCKING_SYNC, g_oDevice));
  }

  // Initialize CUDA related Driver API
  // Determine if we are running on a 32-bit or 64-bit OS and choose the right
  // PTX file
  try {
    if (sizeof(void *) == 4) {
      g_pCudaModule =
          new CUmoduleManager("NV12ToARGB_drvapi32.ptx", exec_path, 2, 2, 2);
    } else {
      g_pCudaModule =
          new CUmoduleManager("NV12ToARGB_drvapi64.ptx", exec_path, 2, 2, 2);
    }
  } catch (char const *p_file) {
    // If the CUmoduleManager constructor fails to load the PTX file, it will
    // throw an exception
  ...
  if (g_bUseInterop) {
    ...
    checkCudaErrors(cuMemAlloc(&g_pInteropFrame[0],
                               g_pVideoDecoder->targetWidth() *
                                   g_pVideoDecoder->targetHeight() * 2));
    checkCudaErrors(cuMemAlloc(&g_pInteropFrame[1],
                               g_pVideoDecoder->targetWidth() *
                                   g_pVideoDecoder->targetHeight() * 2));
  }
...
bool reinitCudaResources() {
  ...
  g_bIsProgressive =
      loadVideoSource(sFileName.c_str(), g_nVideoWidth, g_nVideoHeight,
...
void displayHelp() {
  ...
  printf("\t-decodecuda   - Use CUDA for MPEG-2 (Available with 64+ CUDA "
         "cores)\n");
...
void keyboard(unsigned char key, int x, int y) {
  ...
  case 'f':
    ...
    if (g_pImageGL)
      g_pImageGL->setTextureFilterMode(
...
bool initGL(int argc, char **argv, int *pbTCC) {
  ...
  // If deviceID == 0, and there is more than 1 device, let's find the first
  // available graphics GPU
  if (!bSpecifyDevice && device_count > 0) {
    for (int i = 0; i < device_count; i++) {
      ...
      if (!bSupported) {
        printf("  -> GPU: \"%s\" does not meet the minimum spec of SM 1.1\n",
               device_name);
        printf("  -> A GPU with a minimum compute capability of SM 1.1 or "
               "higher is required.\n");
        return false;
      }
      ...
      checkCudaErrors(
          cuDeviceGetAttribute(pbTCC, CU_DEVICE_ATTRIBUTE_TCC_DRIVER, dev));
#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
      ...
      // We don't know for sure if this is a TCC device or not, if it is Tesla
      // we will not run
    ...
    if ((g_DeviceID > (device_count - 1)) || (g_DeviceID < 0)) {
      printf(" >>> Invalid GPU Device ID=%d specified, only %d GPU device(s) "
             "are available.<<<\n",
             g_DeviceID, device_count);
      printf(
          " >>> Valid GPU ID (n) range is between [%d,%d]...  Exiting... <<<\n",
          0, device_count - 1);
      return false;
    }
    ...
    // We don't know for sure if this is a TCC device or not, if it is Tesla we
    // will not run
    if (!STRNCASECMP(device_name, "Tesla", 5)) {
      printf("  \"%s\" does not support %s\n", device_name, sSDKname);
      *pbTCC = 1;
...
bool initGLTexture(unsigned int nWidth, unsigned int nHeight) {
  g_pImageGL = new ImageGL(nWidth, nHeight, nWidth, nHeight, g_bIsProgressive,
...
bool loadVideoSource(const char *video_file, unsigned int &width,
  ...
  std::auto_ptr<VideoSource> apVideoSource(
      new VideoSource(video_file, apFrameQueue.get()));
  ...
  if (g_bFrameRepeat) {
    g_iRepeatFactor =
        (int)(60.0f /
              ceil((float)apVideoSource->format().frame_rate.numerator /
                   (float)apVideoSource->format().frame_rate.denominator));
...
void initCudaVideo() {
  ...
  std::auto_ptr<VideoDecoder> apVideoDecoder(new VideoDecoder(
      g_pVideoSource->format(), g_oContext, g_eVideoCreateFlags, g_CtxLock));
  std::auto_ptr<VideoParser> apVideoParser(
      new VideoParser(apVideoDecoder.get(), g_pFrameQueue, &g_oContext));
...
// Run the Cuda part of the computation (if g_pFrameQueue is empty, then return
// false)
bool copyDecodedFrameToTexture(unsigned int &nRepeats, int bUseInterop,
                               int *pbIsProgressive) {
  ...
  if (g_pFrameQueue->dequeue(&oDisplayInfo)) {
    ...
    int num_fields = (oDisplayInfo.progressive_frame
                          ? (1)
                          : (2 + oDisplayInfo.repeat_first_field));
    *pbIsProgressive = oDisplayInfo.progressive_frame;
    ...
    for (int active_field = 0; active_field < num_fields; active_field++) {
      ...
      oVideoProcessingParameters.progressive_frame =
      ...
      g_pVideoDecoder->mapFrame(oDisplayInfo.picture_index,
                                &pDecodedFrame[active_field], &nDecodedPitch,
      ...
      // If we are Encoding and this is the 1st Frame, we make sure we allocate
      // system memory for readbacks
      if (g_bReadback && g_bFirstFrame && g_ReadbackSID) {
        CUresult result;
        checkCudaErrors(result =
                            cuMemAllocHost((void **)&g_bFrameData[0],
                                           (nDecodedPitch * nHeight * 3 / 2)));
        checkCudaErrors(result =
                            cuMemAllocHost((void **)&g_bFrameData[1],
                                           (nDecodedPitch * nHeight * 3 / 2)));
      ...
      // If streams are enabled, we can perform the readback to the host while
      // the kernel is executing
      if (g_bReadback && g_ReadbackSID) {
        CUresult result = cuMemcpyDtoHAsync(
            g_bFrameData[active_field], pDecodedFrame[active_field],
            (nDecodedPitch * nHeight * 3 / 2), g_ReadbackSID);
      ...
      printf("%s = %02d, PicIndex = %02d, OutputPTS = %08d\n",
             ...
             g_DecodeFrameCount, oDisplayInfo.picture_index,
             oDisplayInfo.timestamp);
#endif

      if (g_pImageGL) {
        // map the texture surface
        g_pImageGL->map(&pInteropFrame[active_field], &pFramePitch,
                        active_field);
      ...
      // conversion and post processing) comment this out if we inclue the line
      // of code seen above

      cudaPostProcessFrame(&pDecodedFrame[active_field], nDecodedPitch,
                           &pInteropFrame[active_field], pFramePitch,
                           g_pCudaModule->getModule(), g_kernelNV12toARGB,
                           g_KernelSID);
      ...
      // unmap video frame
      // unmapFrame() synchronizes with the VideoDecode API (ensures the frame
  ...
  if (g_pFrameQueue->isDecodeFinished()) {
    // Let's just stop, and allow the user to quit, so they can at least see the
    // results
    // g_pVideoSource->stop();
...
// This is the CUDA stage for Video Post Processing.  Last stage takes care of
// the NV12 to ARGB
void cudaPostProcessFrame(CUdeviceptr *ppDecodedFrame, size_t nDecodedPitch,
                          CUdeviceptr *ppInteropFrame, size_t pFramePitch,
                          CUmodule cuModNV12toARGB, CUfunction fpCudaKernel,
  ...
  eResult = cudaLaunchNV12toARGBDrv(*ppDecodedFrame, nDecodedPitch,
                                    *ppInteropFrame, pFramePitch, nWidth,
                                    nHeight, fpCudaKernel, streamID);
}
...
bool renderVideoFrame(int bUseInterop) {
  ...
  if (0 != g_pFrameQueue) {
    ...
    if (!g_bDeviceLost && g_bRunning) {
      bFramesDecoded =
          copyDecodedFrameToTexture(nRepeatFrame, bUseInterop, &bIsProgressive);
    }
  } else {
    return true;
  }
