...
extern "C" CUresult updateConstantMemory_drvapi(CUmodule module,
  ...
  cuModuleGetGlobal(&d_constHueCSC, &d_cscBytes, module,
                    "constHueColorSpaceMat");
...
extern "C" CUresult cudaLaunchNV12toARGBDrv(CUdeviceptr d_srcNV12,
  ...
  status = cuLaunchKernel(fpFunc, grid.x, grid.y, grid.z, block.x, block.y,
                          block.z, 0, streamID, args, NULL);
#else
  ...
  // This method calls cuParamSetv() to pass device pointers also allows the
  // ability to pass 64-bit device pointers
  ...
  cutilDrvSafeCall(
      cuParamSetv(fpFunc, offset, &nSourcePitch, sizeof(nSourcePitch)));
  ...
  cutilDrvSafeCall(
      cuParamSetv(fpFunc, offset, &nDestPitch, sizeof(nDestPitch)));
  ...
  if (CUDA_SUCCESS != status) {
    fprintf(stderr,
            "cudaLaunchNV12toARGBDrv() failed to launch Kernel Function %p, "
            "retval = %d\n",
            fpFunc, status);
    return status;
  }
