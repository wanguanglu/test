...
__device__ uint32 RGBAPACK_8bit(float red, float green, float blue,
                                uint32 alpha) {
  uint32 ARGBpixel = 0;
  ...
  ARGBpixel = (((uint32)blue) | (((uint32)green) << 8) | (((uint32)red) << 16) |
               (uint32)alpha);

  return ARGBpixel;
}

__device__ uint32 RGBAPACK_10bit(float red, float green, float blue,
                                 uint32 alpha) {
  uint32 ARGBpixel = 0;
  ...
  // Convert to 8 bit unsigned integers per color component
  ARGBpixel = (((uint32)blue >> 2) | (((uint32)green >> 2) << 8) |
...
extern "C" __global__ void Passthru_drvapi(uint32* srcImage,
  ...
  dstImage[y * dstImagePitch + x] =
      RGBAPACK_8bit(luma[0], luma[0], luma[0], constAlpha);
  dstImage[y * dstImagePitch + x + 1] =
      RGBAPACK_8bit(luma[1], luma[1], luma[1], constAlpha);
}
...
extern "C" __global__ void NV12ToARGB_drvapi(uint32* srcImage,
  ...
  if (y & 1)  // odd scanline ?
  {
    ...
    yuv101010Pel[0] |=
        ((uint32)srcImageU8[chromaOffset + y_chroma * processingPitch + x]
         << (COLOR_COMPONENT_BIT_SIZE + 2));
    yuv101010Pel[0] |=
        ((uint32)srcImageU8[chromaOffset + y_chroma * processingPitch + x + 1]
  ...
  // Clamp the results to RGBA
  dstImage[y * dstImagePitch + x] =
      RGBAPACK_10bit(red[0], green[0], blue[0], constAlpha);
  dstImage[y * dstImagePitch + x + 1] =
      RGBAPACK_10bit(red[1], green[1], blue[1], constAlpha);
}
