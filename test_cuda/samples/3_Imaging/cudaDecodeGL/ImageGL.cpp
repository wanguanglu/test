...
ImageGL::ImageGL(unsigned int nDispWidth, unsigned int nDispHeight,
  ...
  for (int n = 0; n < nFrames; n++) {
    ...
    glBufferDataARB(GL_PIXEL_UNPACK_BUFFER_ARB, nTexWidth * nTexHeight * 4,
...
void ImageGL::registerAsCudaResource(int field_num) {
...
void ImageGL::unregisterAsCudaResource(int field_num) {
...
void ImageGL::setTextureFilterMode(GLuint nMINfilter, GLuint nMAGfilter) {
  ...
  for (int field_num = 0; field_num < nFrames; field_num++) {
    ...
    glTexImage2D(GL_TEXTURE_TYPE, 0, GL_RGBA8, nTexWidth_, nTexHeight_, 0,
...
void ImageGL::unmap(int field_num) {
...
void ImageGL::clear(unsigned char nClearColor) {
  ...
  for (int field_num = 0; field_num < nFrames; field_num++) {
    ...
    checkCudaErrors(
        cuMemsetD8(pImageData, nClearColor, nTexWidth_ * nTexHeight_ * Bpp()));
    unmap(field_num);
  }
}

void ImageGL::render(int field_num) const {
  // Common display code path
  {
    ...
    glBindTexture(GL_TEXTURE_TYPE, gl_texid_[field_num]);
    glTexSubImage2D(GL_TEXTURE_TYPE, 0, 0, 0, nTexWidth_, nTexHeight_, GL_BGRA,
                    GL_UNSIGNED_BYTE, 0);
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, 0);
