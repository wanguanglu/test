...
//! images is computed against different horizontal shifts of areas from the
//! right. The shift at which the difference is minimum is taken as how far that
//! pixel moved between left/right image pairs.   The recovered motion is the
//! disparity map More motion indicates more parallax indicates a closer object.
...
#pragma unroll
    ...
    for (int i = 0; i < STEPS; i++) {
      ...
      if (threadIdx.x < 2 * RAD) {
        ...
        imLeft = imLeftB[i];
        imRight =
            tex2D(tex2Dright, tidx - RAD + blockSize_x + d, tidy + offset);
        cost = __usad4(imLeft, imRight);
