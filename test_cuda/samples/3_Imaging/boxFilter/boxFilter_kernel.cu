...
/*
   ...
   left. This only requires 2 adds and a mul per output value, independent of
   the filter radius. The box filter is separable, so to perform a 2D box filter
   we perform the filter in the x direction, followed by the same filter in the
   y direction. Applying multiple iterations of the box filter converges towards
   a Gaussian blur. Using CUDA, rows or columns of the image are processed in
...
extern "C" double boxFilter(float* d_src, float* d_temp, float* d_dest,
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    d_boxfilter_x_tex<<<height / nthreads, nthreads, 0>>>(d_temp, width, height,
                                                          radius);
    ...
    if (iterations > 1) {
      ...
      checkCudaErrors(cudaMemcpyToArray(d_tempArray, 0, 0, d_dest,
...
extern "C" double boxFilterRGBA(unsigned int* d_src, unsigned int* d_temp,
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    // use texture for horizontal pass
    d_boxfilter_rgba_x<<<height / nthreads, nthreads, 0>>>(d_temp, width,
                                                           height, radius);
